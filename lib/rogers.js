...
EntityButtonEnclave = EntityButton.extend({
	...
	update: function() {
		if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
			window.top.location.href = 'http://www.didacus.epizy.com';
		}
	}
});
EntityButtonBlackmoon = EntityButton.extend({
	...
	update: function() {
		if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
			window.top.location.href = 'https://www.instagram.com/didacusamorim/';
		}
	}
});
...
EntityButtonFacebook = EntityButton.extend({
	...
	update: function() {
		if(ig.input.pressed('click') && this.inFocus()) {
			if(ig.Sound.enabled)
				this.sound.play();
			window.top.location.href = 'https://www.facebook.com/didacusamorim/';
		}
	}
});
...
StartScreen = ig.Game.extend({
	...
	init: function() {
		...
		ig.music.add('media/audio/theme-music.mp3');
		ig.music.volume = 0.7;
		if(ig.Sound.enabled) {
			ig.music.play();
			this.BtnAudio.currentAnim = this.BtnAudio.anims['true'];
